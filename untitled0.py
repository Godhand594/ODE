...
'''定義Lothenbach模型'''
def Rt_NG(t,AlfaNG):
    return C3S['K1']/C3S['N1']*(1-AlfaNG)*(-torch.log(1-AlfaNG))**(1-C3S['N1'])*((rh-0.55)/0.45)**(4) \
        *surface_area/Reference_area \
        *torch.exp(torch.tensor(C3S['Ea'])*(1/Reference_T-1/T)/R)

def Rt_D(t,AlfaD):
    return C3S['K2']*(1-AlfaD)**(2/3)/(1-(1-AlfaD)**(1/3)) \
        *((rh-0.55)/0.45)**(4) \
        *torch.exp(torch.tensor(C3S['Ea'])*(1/Reference_T-1/T)/R)
    
def Rt_FYS(t,AlfaFYS):
    return C3S['K3']*(1-AlfaFYS)**(C3S['N3']) \
        *((rh-0.55)/0.45)**(4) \
        *torch.exp(torch.tensor(C3S['Ea'])*(1/Reference_T-1/T)/R)
...
def ODE(x,y):
    ...
    maxma,_ = torch.min(torch.cat((Rt_NG(x,y),Rt_D(x,y),Rt_FYS(x,y)),1),1)
    eq = dydx-maxma
